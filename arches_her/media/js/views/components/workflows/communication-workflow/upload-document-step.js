...
function viewModel(params) {
        ...
        const saveDigitalResourceName = async() => {
            ...
            const consultation = await window.fetch(generateArchesURL('arches:api_resources')(ko.unwrap(self.consultationResourceId)) + '?format=json');
            if (consultation?.ok) {
                ...
                const nameTile = await window.fetch(generateArchesURL('arches:api_tiles')(self.digitalResourceNameTileId), {
                    method: 'POST',
                    credentials: 'include',
        ...
        const saveConsultationRelationship = async() =>  {
            ...
            return await window.fetch(generateArchesURL('arches:api_node_value'), {
                method: 'POST',
                credentials: 'include',
                body: formData,
            });
